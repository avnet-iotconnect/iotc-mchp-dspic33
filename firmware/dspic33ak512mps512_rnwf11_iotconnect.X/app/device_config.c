...
bool DEVICE_CONFIG_Save(device_config_t *cfg)
{
    ...
    // 16-byte Flash word size; pad the tail with zeros in a scratch buffer
    // rather than requiring device_config_t itself to be pre-sized to a
    // multiple of 16 (the struct's layout may shift across future field
    // additions/compiler versions).
    uint16_t padded_len = (uint16_t)((sizeof(device_config_t) + (NVM_FLASH_WORD_SIZE - 1U)) & ~(NVM_FLASH_WORD_SIZE - 1U));
    uint8_t buf[((sizeof(device_config_t) + NVM_FLASH_WORD_SIZE - 1U) / NVM_FLASH_WORD_SIZE) * NVM_FLASH_WORD_SIZE];
